...
describe('PhieuThuHangThangService', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            providers: [
                phieu_thu_hang_thang_service_1.PhieuThuHangThangService,
                { provide: prisma_service_1.PrismaService, useValue: mockPrisma },
            ],
        }).compile();
        service = module.get(phieu_thu_hang_thang_service_1.PhieuThuHangThangService);
        jest.clearAllMocks();
    });
    ...
    describe('findOne()', () => {
        it('trả về record khi tìm thấy', async () => {
            ...
            expect(result).toEqual(MOCK_ITEM);
            expect(mockPrisma.phieuThuHangThang.findUnique).toHaveBeenCalledWith(expect.objectContaining({ where: { maPhieuThu: VALID_ID } }));
        });
    ...
    describe('create()', () => {
        it('tạo mới và trả về record', async () => {
            ...
            expect(result).toEqual(MOCK_ITEM);
            expect(mockPrisma.phieuThuHangThang.create).toHaveBeenCalledWith(expect.objectContaining({ data: CREATE_DTO }));
        });
    ...
    describe('update()', () => {
        it('cập nhật và trả về record đã sửa', async () => {
            ...
            expect(mockPrisma.phieuThuHangThang.update).toHaveBeenCalledWith(expect.objectContaining({ where: { maPhieuThu: VALID_ID } }));
        });
    ...
    describe('remove()', () => {
        it('xóa và trả về record đã xóa', async () => {
            ...
            expect(result).toEqual(MOCK_ITEM);
            expect(mockPrisma.phieuThuHangThang.delete).toHaveBeenCalledWith({ where: { maPhieuThu: VALID_ID } });
        });
        it('ném NotFoundException khi record không tồn tại', async () => {
            mockPrisma.phieuThuHangThang.findUnique.mockResolvedValue(null);
...
//# sourceMappingURL=phieu-thu-hang-thang.service.spec.js.map