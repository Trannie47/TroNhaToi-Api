...
describe('HoaDonGuiXeService', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            providers: [
                hoa_don_gui_xe_service_1.HoaDonGuiXeService,
                { provide: prisma_service_1.PrismaService, useValue: mockPrisma },
            ],
        }).compile();
        service = module.get(hoa_don_gui_xe_service_1.HoaDonGuiXeService);
        jest.clearAllMocks();
    });
    ...
    describe('findOne()', () => {
        it('trả về record khi tìm thấy', async () => {
            ...
            expect(result).toEqual(MOCK_ITEM);
            expect(mockPrisma.hoaDonGuiXe.findUnique).toHaveBeenCalledWith(expect.objectContaining({ where: { maHoaDon: VALID_ID } }));
        });
    ...
    describe('create()', () => {
        it('tạo mới và trả về record', async () => {
            ...
            expect(result).toEqual(MOCK_ITEM);
            expect(mockPrisma.hoaDonGuiXe.create).toHaveBeenCalledWith(expect.objectContaining({ data: CREATE_DTO }));
        });
    ...
    describe('update()', () => {
        it('cập nhật và trả về record đã sửa', async () => {
            ...
            expect(mockPrisma.hoaDonGuiXe.update).toHaveBeenCalledWith(expect.objectContaining({ where: { maHoaDon: VALID_ID } }));
        });
    ...
    describe('remove()', () => {
        it('xóa và trả về record đã xóa', async () => {
            ...
            expect(result).toEqual(MOCK_ITEM);
            expect(mockPrisma.hoaDonGuiXe.delete).toHaveBeenCalledWith({ where: { maHoaDon: VALID_ID } });
        });
        it('ném NotFoundException khi record không tồn tại', async () => {
            mockPrisma.hoaDonGuiXe.findUnique.mockResolvedValue(null);
...
//# sourceMappingURL=hoa-don-gui-xe.service.spec.js.map