...
let HopDongService = class HopDongService {
    ...
    async findOne(id) {
        ...
        if (!item)
            throw new common_1.NotFoundException(`HopDong với id ${id} không tồn tại`);
        return item;
    }
    create(dto) {
        return this.prisma.hopDong.create({ data: dto });
    }
    async update(id, dto) {
        await this.findOne(id);
        return this.prisma.hopDong.update({ where: { hopDongId: id }, data: dto });
    }
    async remove(id) {
        await this.findOne(id);
