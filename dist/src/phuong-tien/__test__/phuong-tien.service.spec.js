...
describe('PhuongTienService', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            providers: [
                phuong_tien_service_1.PhuongTienService,
                { provide: prisma_service_1.PrismaService, useValue: mockPrisma },
            ],
        }).compile();
        service = module.get(phuong_tien_service_1.PhuongTienService);
        jest.clearAllMocks();
    });
    ...
    describe('findOne()', () => {
        it('trả về record khi tìm thấy', async () => {
            ...
            expect(result).toEqual(MOCK_ITEM);
            expect(mockPrisma.phuongTien.findUnique).toHaveBeenCalledWith(expect.objectContaining({ where: { bienSo: VALID_ID } }));
        });
    ...
    describe('create()', () => {
        it('tạo mới và trả về record', async () => {
            ...
            expect(result).toEqual(MOCK_ITEM);
            expect(mockPrisma.phuongTien.create).toHaveBeenCalledWith(expect.objectContaining({ data: CREATE_DTO }));
        });
    ...
    describe('update()', () => {
        it('cập nhật và trả về record đã sửa', async () => {
            ...
            expect(mockPrisma.phuongTien.update).toHaveBeenCalledWith(expect.objectContaining({ where: { bienSo: VALID_ID } }));
        });
    ...
    describe('remove()', () => {
        it('xóa và trả về record đã xóa', async () => {
            ...
            expect(result).toEqual(MOCK_ITEM);
            expect(mockPrisma.phuongTien.delete).toHaveBeenCalledWith({ where: { bienSo: VALID_ID } });
        });
        it('ném NotFoundException khi record không tồn tại', async () => {
            mockPrisma.phuongTien.findUnique.mockResolvedValue(null);
...
//# sourceMappingURL=phuong-tien.service.spec.js.map